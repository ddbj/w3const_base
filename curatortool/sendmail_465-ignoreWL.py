...
# This mail sender ingores whitelist file.
...
#host, port = 'gw1', 25
host, port = 'mail1i', 465
accnt = ['no-reply@ddbj.nig.ac.jp', '']

#Ignore host cerfity
context = ssl.create_default_context()
context.check_hostname = False  # ホスト名の検証を無効化
context.verify_mode = ssl.CERT_NONE  # 証明書の検証を無効化smtplib.SMTP内に追加する
...
def sendemail(account, pwd, subject, sendfrom, to, body, cc, bcc, attach):
    ...
    #
    try:
        with smtplib.SMTP_SSL(host, port, context=context, timeout=30) as gmail:
            gmail.ehlo()
#            gmail.starttls()
